...
工具中心和设置页负责日常维护：查看 Hermes toolsets、补齐外部 Provider、运行诊断、检查更新、管理备份以及安全卸载。它们属于日常使用入口，不是内部测试页。
...
工具中心提供 Hermes 更新检查。默认更新会按 Hermes updater 当前策略执行；如果选择完整备份，更新前会先创建备份，耗时取决于工作空间和资源大小。

更新完成后，工具中心会刷新 tools list、Doctor 缓存和 Provider 列表。
